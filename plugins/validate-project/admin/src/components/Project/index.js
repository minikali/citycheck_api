...
const translate = async (txt, destLang) => {
  try {
    const key =
      "trnsl.1.1.20200410T111243Z.09305873700877ce.821f1f457e4ac22fee5abcdfaeca67f5949e1c3f";
    ...
    const response = await fetch(
      `https://translate.yandex.net/api/v1.5/tr.json/translate?key=${key}&lang=${destLang}`,
      requestOptions
    );
...
const Project = (props) => {
  ...
  useEffect(() => {
    (async () => {
      const title =
        state.title &&
        (await translate(state.title, props.lang === "fr" ? "en" : "fr"))
          .text[0];
      const description =
        state.description &&
        (await translate(state.description, props.lang === "fr" ? "en" : "fr"))
          .text[0];
      const justify =
        state.justify &&
        (await translate(state.justify, props.lang === "fr" ? "en" : "fr"))
          .text[0];
      const t = {
        title: title,
        phase: state.phase,
      ...
      <div className={"container-fluid"} style={{ padding: "18px 30px" }}>
        <div className="row">
          <div className={"col-6"}>
            ...
            />
            <Label htmlFor="phase" style={{ marginTop: "10px" }}>
              Phase
            </Label>
            <Select
              name="phase"
            ...
            >{`Address [${props.lat},${props.lng}]`}</Label>
            <InputText
              id={state.id}
              name={"address"}
