...
const UploadFileForm = ({ onRequestAnalysis, loadingAnalysis }) => {
  ...
  const onChangeImportFile = file => {
    file &&
      setState({
        file,
        type: file.type,
        options: { ...state.options, filename: file.name }
      });
  };

  const handleDragEnter = () => setState({ ...state, isDragging: true });

  const handleDragLeave = () => setState({ ...state, isDragging: false });
  ...
  const clickAnalyzeUploadFile = async () => {
  ...
  return (
    <div className={"col-12"}>
        ...
        >
          ...
          <input
            ...
            onChange={({ target: { files } }) =>
              files && onChangeImportFile(files[0])
            }
            type="file"
          />
        </Label>
      ...
      <Row className={"row"}>
        <Button
          label={"Analyze"}
          color={state.file ? "secondary" : "cancel"}
          disabled={!state.file}
